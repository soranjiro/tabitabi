...
<div class="ss-week-view">
  ...
  <div class="ss-calendar-grid ss-week-grid">
    {#each DAY_NAMES as name, i}
      {@const day = weekDays()[i]}
      {@const dateStr = day ? toDateStr(day) : ''}
      {@const daySteps = dateStr ? getStepsForDate(dateStr) : []}
      <div class="ss-calendar-cell ss-calendar-cell--week">
        <div class="ss-calendar-day-header">
          <span class="ss-calendar-day-name">{name}</span>
          {#if day}<span class="ss-calendar-day-num">{day.getDate()}</span>{/if}
        </div>
        <div class="ss-calendar-cell-steps">
          {#each daySteps as step (step.id)}
            ...
            >
              <span class="ss-calendar-step-time">{step.time}</span>
              <span class="ss-calendar-step-title">{step.title}</span>
            </div>
          {/each}
        </div>
      </div>
