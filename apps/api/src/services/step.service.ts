...
function normalizeLegacyNotes(
  notes: string,
  state: Pick<Step, 'start_at' | 'time_unspecified' | 'sort_order' | 'pin_latitude' | 'pin_longitude' | 'is_priority' | 'link'>,
  ...
  if (state.pin_latitude !== null && state.pin_latitude !== undefined
    ...
    data.tabitabi_place = { priority: Boolean(state.is_priority) };
  }
  if (state.link !== undefined && Object.hasOwn(data, 'booking_url')) data.booking_url = state.link;
...
export class StepService {
  ...
  async update(stepId: string, input: UpdateStepInput): Promise<Step | null> {
    ...
    const nextLegacyNotes = normalizeLegacyNotes(sourceNotes, {
      ...
      link: input.link === undefined ? existing.link : normalizeLink(input.link),
    }, existingRow.notes);
