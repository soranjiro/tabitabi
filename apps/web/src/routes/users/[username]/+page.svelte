<script lang="ts">
  ...
  const pageTitle = $derived(notFound ? "ユーザーが見つかりません" : `@${username}`);
  const pageSubtitle = $derived(!loading && !notFound ? "公開しおり一覧" : undefined);
...
<PageShell title={pageTitle} subtitle={pageSubtitle} backHref="/users" backLabel="← みんなのしおり">
  {#snippet children()}
      {#if loading}
