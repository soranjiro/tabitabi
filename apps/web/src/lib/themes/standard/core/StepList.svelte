...
{#if (viewMode === "week" || viewMode === "month") && steps.some(step => step.start_at === null)}
  <ListView steps={steps.filter(step => step.start_at === null)}
    {hasEditPermission} {secretModeEnabled} {secretModeOffset}
    {onUpdateStep} {onDeleteStep} {onOpenDateEditor} />
{/if}
