...
WITH itinerary_seed(season, title, theme_id, palette_id, prefecture_slugs, areas, tags, memo) AS (
  VALUES
    ('spring', '桜の京都・宇治', 'daycard', 'sakura', '["kyoto"]', '["東山","嵐山","宇治","伏見"]', '["桜","寺社・歴史","グルメ"]', '{"text":"4月2日（金）から2泊3日。朝は早めに出発し、混みやすい名所は午前中へ。宿は京都駅近くなので、到着日と最終日の荷物はホテルに預ける。歩く時間が長いため、履き慣れた靴で集合。"}'),
    ('summer', '夏休みの沖縄旅行', 'list', 'ocean', '["okinawa"]', '["恩納村","本部町","やんばる","那覇"]', '["海","自然","グルメ"]', '{"text":"7月16日（金）から3泊4日。那覇空港でレンタカーを受け取り、恩納村に3連泊する。海の予定は天候と海況を見て変更し、雨天時は水族館や首里城公園を先に回る。運転は1〜2時間ごとに交代する。"}'),
    ('autumn', '日光・会津 紅葉と温泉', 'week', 'autumn', '["tochigi","fukushima"]', '["日光","奥日光","中禅寺湖","鬼怒川","会津若松"]', '["紅葉","温泉","自然","寺社・歴史"]', '{"text":"10月19日（月）から6泊7日。日光駅までは鉄道、その先はバスと会津鬼怒川線で移動する。奥日光は朝晩冷え込むため薄手のダウンを持参。宿は日光1泊、中禅寺温泉2泊、鬼怒川温泉1泊、会津若松2泊。"}'),
    ('winter', '冬の北海道 湯めぐり18日間', 'month', 'snow', '["hokkaido"]', '["札幌","小樽","余市","ニセコ","洞爺湖","登別","函館"]', '["雪景色","温泉","グルメ","長期旅行"]', '{"text":"2月1日（月）から17泊18日。札幌から函館へ鉄道と送迎バスで南下する。吹雪で移動が遅れる場合に備え、各滞在地に予備時間を確保。防水の滑りにくい靴、防寒手袋、モバイルバッテリーを忘れずに。"}')
)
