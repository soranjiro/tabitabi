<script lang="ts">
  ...
  let selectorRoot: HTMLDivElement;
  let open = $state(false);
  let query = $state("");
  let opensUpward = $state(false);
  let desktopPanelMaxHeight = $state(448);
  ...
  function toggleOpen() {
    ...
    if (typeof window !== "undefined" && selectorRoot) {
      const rect = selectorRoot.getBoundingClientRect();
      ...
      opensUpward = spaceBelow < 360 && spaceAbove > spaceBelow;
      desktopPanelMaxHeight = Math.min(448, opensUpward ? spaceAbove : spaceBelow);
    }
...
<div class="prefecture-selector" bind:this={selectorRoot}>
  ...
  {#if open}
    ...
    <div
      class="panel"
      class:open-upward={opensUpward}
      role="dialog"
      aria-modal="true"
      aria-label={ariaLabel}
      style:--desktop-panel-max-height={`${desktopPanelMaxHeight}px`}
    >
...
<style>
  ...
  .panel { position: absolute; z-index: 1100; top: calc(100% + .45rem); left: 0; display: flex; width: 100%; max-height: var(--desktop-panel-max-height, 28rem); box-sizing: border-box; overflow: hidden; flex-direction: column; border: 1px solid var(--theme-border, #dbe1e8); border-radius: .9rem; color: var(--theme-text, #334155); background: white; box-shadow: 0 18px 48px rgba(35, 48, 73, .18); }
  .panel.open-upward { top: auto; bottom: calc(100% + .45rem); }
  .panel-header { display: flex; flex: 0 0 auto; padding: .85rem .9rem .65rem; align-items: center; justify-content: space-between; gap: .75rem; }
  ...
  .search { display: block; width: calc(100% - 1.8rem); height: 2.75rem; flex: 0 0 auto; box-sizing: border-box; margin: 0 .9rem .65rem; padding: 0 .8rem; border: 1px solid var(--theme-border, #dbe1e8); border-radius: .65rem; color: var(--theme-text, #334155); background: #f8fafc; font: inherit; font-size: 16px; }
  .search:focus { outline: 2px solid var(--theme-primary, #6366f1); outline-offset: 1px; }
  .groups { min-height: 0; overflow-y: auto; flex: 1 1 auto; padding: 0 .9rem .8rem; overscroll-behavior: contain; }
  ...
  .panel-footer { display: flex; flex: 0 0 auto; padding: .7rem .9rem; border-top: 1px solid var(--theme-border, #e5e7eb); align-items: center; justify-content: space-between; gap: .75rem; background: #fff; }
  ...
  @media (max-width: 640px) {
    ...
    .panel, .panel.open-upward { position: fixed; top: auto; right: 0; bottom: 0; left: 0; width: 100%; max-height: min(82dvh, 42rem); border-width: 1px 0 0; border-radius: 1.05rem 1.05rem 0 0; box-shadow: 0 -16px 48px rgba(30, 41, 59, .22); padding-bottom: env(safe-area-inset-bottom); }
