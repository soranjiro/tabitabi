<script lang="ts">
  ...
  async function saveItem() {
    ...
    if (!name) return;
    if (!groupId) return;
    const quantity = Math.max(1, Math.min(999, Math.trunc(itemQuantity || 1)));
