<script lang="ts">
  ...
  import { prefectures as explorePrefectures } from "$lib/explore/data";
  import { type Prefecture, type PublicFeedItem, type UserBookmarkWithItinerary, type UserSessionProfile } from "@tabitabi/types";

  type Mode = "login" | "register" | "verify" | "forgot" | "setup";
  const usernamePattern = "[\\p{L}\\p{N}\\p{M}_]+";
  const profilePrefectureOptions = explorePrefectures.map((item) => ({ value: item.name, label: item.name, region: item.region }));
  ...
  function setPrefecture(values: string[]) {
    prefecture = (values[0] as Prefecture | undefined) ?? "";
  }

  function setEditPrefecture(values: string[]) {
    editPrefecture = (values[0] as Prefecture | undefined) ?? "";
  }
  ...
  {#snippet children()}
    ...
    {:else if !loggedIn}
      <section class="max-w-md mx-auto rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-50 via-white to-sky-50 shadow-sm p-6 sm:p-8">
        ...
        {:else if mode === "setup"}
          <form onsubmit={(event) => { event.preventDefault(); completeProfile(); }} class="space-y-4"><p class="text-sm text-gray-600">メール確認が完了しました。公開名とお住まいの地域を設定してください。</p><label for="setup-username">ユーザー名</label><input id="setup-username" bind:value={usernameInput} minlength="3" maxlength="20" pattern={usernamePattern} title="3〜20文字の日本語・英数字・_が使えます" autocomplete="username" required /><label>お住まいの都道府県</label><PrefectureSelector options={profilePrefectureOptions} selectedValues={prefecture ? [prefecture] : []} placeholder="お住まいの都道府県を選ぶ" ariaLabel="お住まいの都道府県" onChange={setPrefecture} /><p class="text-xs text-gray-500">都道府県は旅の傾向改善に利用し、公開プロフィールには表示しません。</p><button type="submit" disabled={submitting} class="primary">{submitting ? "設定中..." : "利用を開始する"}</button></form>
        {:else}
          <form onsubmit={(event) => { event.preventDefault(); handleAuthSubmit(); }} class="space-y-4">
            {#if mode === "register"}<label for="username">ユーザー名</label><input id="username" bind:value={usernameInput} minlength="3" maxlength="20" pattern={usernamePattern} title="3〜20文字の日本語・英数字・_が使えます" autocomplete="username" required />{/if}
            ...
            {#if mode === "register"}<label>お住まいの都道府県</label><PrefectureSelector options={profilePrefectureOptions} selectedValues={prefecture ? [prefecture] : []} placeholder="お住まいの都道府県を選ぶ" ariaLabel="お住まいの都道府県" onChange={setPrefecture} /><p class="text-xs text-gray-500">公開されないプロフィール情報です。</p>{/if}
    ...
    {:else}
      ...
      {#if showAccount}<section class="account-card">
        ...
        {#if editSection !== "none"}<div class="mt-5 pt-5 border-t">{#if editError}<p class="mb-3 text-sm text-red-600">{editError}</p>{/if}
          {#if editSection === "profile"}<form onsubmit={(event) => { event.preventDefault(); updateProfile(); }} class="space-y-3"><label for="edit-username">ユーザー名</label><input id="edit-username" bind:value={editUsername} minlength="3" maxlength="20" pattern={usernamePattern} title="3〜20文字の日本語・英数字・_が使えます" required /><label>お住まいの都道府県</label><PrefectureSelector options={profilePrefectureOptions} selectedValues={editPrefecture ? [editPrefecture] : []} placeholder="お住まいの都道府県を選ぶ" ariaLabel="お住まいの都道府県" onChange={setEditPrefecture} /><div class="actions"><button type="button" onclick={() => editSection = "none"} class="secondary compact">キャンセル</button><button type="submit" disabled={submitting} class="primary compact">保存</button></div></form>
