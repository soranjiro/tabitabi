...
async function setup() {
  const migrations = [
    ...
    `CREATE TABLE IF NOT EXISTS itinerary_packing_items (id TEXT PRIMARY KEY, itinerary_id TEXT NOT NULL, name TEXT NOT NULL, kind TEXT NOT NULL, assignee_member_id TEXT, is_packed INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);`,
...
describe('Packing API', () => {
  ...
  it('shares trip members and keeps personal checks separate per member', async () => {
    ...
    const personalResponse = await app.fetch(new Request(`http://localhost/api/v1/itineraries/${itinerary.id}/packing/items`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'パスポート', kind: 'personal' }),
    }), env);
    ...
    const sharedResponse = await app.fetch(new Request(`http://localhost/api/v1/itineraries/${itinerary.id}/packing/items`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'カメラ', kind: 'shared', assignee_member_id: bob.id }),
    }), env);
    ...
    expect(data.members.map((member: { name: string }) => member.name)).toEqual(['Alice', 'Bob']);
    expect(data.items.find((item: { id: string }) => item.id === personal.id)).toMatchObject({ checked_member_ids: [alice.id], is_packed: false });
